...
This document explains how to create and use a materialized view that stores pre-aggregated monthly metrics derived from the raw `transfers.transfers` table populated by `substreams-sink-sql`.
...
Typical schema (effective types):
- `month` UInt32 (e.g., 202501)
- `volume` same numeric type as `amount` in `transfers.transfers`
